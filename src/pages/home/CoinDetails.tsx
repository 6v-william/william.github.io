...
const CoinDetails: React.FC<CoinDetailsProps> = ({ coinSymbol, isLoading }) => {
  ...
  return (
    ...
    >
      {isLoading || !coinData ? (
        <Skeleton active paragraph={{ rows: 4 }} style={{ borderRadius: '8px' }} />
      ) : error ? (
        <div style={{ color: '#cf1322', fontSize: '16px' }}>{error}</div>
      ) : (
        <>
          <Row gutter={16}>
            <Col span={12}>
            ...
            <Col span={12}>
              <Statistic
                ...
                precision={2}
                valueStyle={{
                  color: coinData.priceChange24h >= 0 ? '#3f8600' : '#cf1322',
                  fontSize: '18px',
                }}
                suffix="%"
                style={{ fontSize: '16px' }}
          ...
          <Row gutter={16} style={{ marginTop: '16px' }}>
            ...
            <Col span={12}>
              <Statistic
                ...
                precision={2}
                valueStyle={{
                  color: getPriceChangePercentage() >= 0 ? '#3f8600' : '#cf1322',
                  fontSize: '18px',
                }}
                suffix="%"
                style={{ fontSize: '16px' }}
